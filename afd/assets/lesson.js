...
(function () {
  ...
  function buildBottomNav(current) {
    ...
    if (next) {
      ...
      var done = el('a', 'pagenav-card pagenav-next',
        ...
        '<span class="pagenav-teaser">Das war die letzte Lektion. Die Referenzdokumente bleiben dein Nachschlagewerk.</span>');
      done.href = indexHref();
      wrap.appendChild(done);
    }

    wrap.appendChild(buildTocLink());
    return wrap;
  }

  // Rueckweg zum Inhaltsverzeichnis — steht unter den beiden Karten und
  // spannt beide Spalten. Auch Referenzseiten bekommen ihn (dort allein).
  function buildTocLink() {
    var a = el('a', 'pagenav-toc',
      '<span class="pagenav-toc-mark" aria-hidden="true">\u2637</span> Inhaltsverzeichnis');
    a.href = indexHref();
    return a;
  }

  function buildRefNav() {
    var wrap = el('nav', 'pagenav is-ref');
    wrap.appendChild(buildTocLink());
    return wrap;
  }
  ...
  // <a class="source" href="...">Quellenangabe</a> steht im Fliesstext mitten im
  // Satz. Hier wird daraus eine hochgestellte Ziffer, und der Klartext wandert in
  // eine Quellenliste an den Fuss der Seite. Reihenfolge = Dokumentreihenfolge.
  function initCitations(host) {
    var cites = host.querySelectorAll('a.source');
    if (!cites.length) return null;

    var wrap = el('section', 'citations');
    wrap.appendChild(el('h2', 'citations-title', 'Quellen'));
    var list = el('ol', 'citelist');

    Array.prototype.forEach.call(cites, function (a, i) {
      var n = i + 1;
      var label = a.innerHTML;
      var href = a.getAttribute('href') || '';

      a.id = 'beleg-' + n;
      a.setAttribute('href', '#quelle-' + n);
      a.setAttribute('title', a.textContent.replace(/\s+/g, ' ').trim());
      a.textContent = n;
      a.classList.add('is-numbered');

      var li = el('li');
      li.id = 'quelle-' + n;

      var link = el('a', 'citelist-link', label);
      link.href = href;
      link.target = '_blank';
      link.rel = 'noopener';
      li.appendChild(link);

      var back = el('a', 'citelist-back', '\u21a9');
      back.href = '#beleg-' + n;
      back.setAttribute('aria-label', 'Zurueck zur Fundstelle im Text');
      li.appendChild(back);
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Quellenliste zuerst — sie gehoert ueber die Weiter-Navigation.
    var citations = initCitations(main);
    if (citations) main.appendChild(citations);

    if (current) {
      main.appendChild(buildBottomNav(current));
    } else if (page === 'reference') {
      main.appendChild(buildRefNav());
    }
